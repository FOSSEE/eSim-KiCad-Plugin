...
try:
    # WINDOWS
    from .board_info import BoardBaselines
    from .utils import * 
...
class PowerIntegrity(pcbnew.ActionPlugin):
    ...
    def Run(self):
        ...
        baseLines = BoardBaselines(board)
        footMaps = baseLines.get_footPrints()
        netGraph = baseLines.get_netInfo()
        nets = baseLines._get_nets()
...
class PIADialog(wx.Dialog):
    def __init__(self, parent, netGraph, nets, footMaps, board, plugin):
        ...
        self.board_path = board.GetFileName()
        ...
        self.clear_btn = wx.Button(panel, label="🗑 Clear Markers")   
        self.clear_btn.Bind(wx.EVT_BUTTON, self.on_clear)             
        vbox.Add(self.clear_btn, 0, wx.CENTER | wx.BOTTOM, 10)        
